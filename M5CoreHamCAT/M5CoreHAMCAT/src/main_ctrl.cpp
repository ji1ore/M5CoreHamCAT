...
void drawMainUI(const String &freq, const String &mode, const String &model, int signalStrength)
{
  ...
  // --- 送信時表示 ---
  canvas.setFont(&fonts::efontJA_12);
  canvas.setCursor(185, 10);
  canvas.setTextColor(isTransmitting ? RED : DARKGREY);
  canvas.print("TX");
  ...
  // --- ボタン描画（横幅最大化） ---
  int btnW = 96, btnH = 36;
  int spacingX = 6, spacingY = 8;
  int startX = (320 - (btnW * 3 + spacingX * 2)) / 2;
  int startY = 110;
...
void handleMainUIScreen()
{
  ...
  if (t.wasReleased())
  {
    ...
    // ★ スワイプは画面下（ボタンエリア）だけ有効

    if (touchStartY < 110)
    {
      return; // 上のエリアではスワイプ無効
    }
    ...
    {
      int dir = (dx > 0) ? +1 : -1;
      int magnitude = abs(dx) / 40;
      if (magnitude < 1)
        magnitude = 1;

      // ★ ボタン選択中 → そのボタンの上でスワイプしたときだけ値変更
      MenuItem touched = detectTouchedButton(t.x, t.y);

      if (selectedItem != MENU_NONE && touched == selectedItem)
      {
        handleSwipe(dir * magnitude);
        drawMainUI(String(lastFreqHz), lastMode, lastModel, signalStrength);
      }
...
RigStatus fetchRigStatus()
{
  ...
  HTTPClient http;
  String url = "http://" + raspiHost + ":" + String(apiPort) + "/radio/status";
  http.begin(url);
  int code = http.GET();
  ...
  if (doc["power"].is<String>())
  {
    ...
    currentPowerNorm = powerStr.toFloat(); // ← これで安全に変換！
  }
...
void handleSwipe(int dir)
{
  ...
  case MENU_POW:
  {
    ...
    HTTPClient http;
    String url = "http://" + raspiHost + ":" + String(apiPort) + "/radio/setpower?value=" + String(powerStr);
    http.begin(url);
    http.GET();
    http.end();
    break;
  ...
  case MENU_SQL:
  {
    ...
    HTTPClient http;
    String url = "http://" + raspiHost + ":" + String(apiPort) + "/radio/setlevel?name=SQL&value=" + String(sqlStr);
    http.begin(url);
    http.GET();
    http.end();
  }
...
void changeFreq(int dir)
{
  ...
  HTTPClient http;
  String url = "http://" + raspiHost + ":" + String(apiPort) + "/radio/setfreq?f=" + String(newFreq);
...
void newFreq(int64_t newFreqHz)
{
  ...
  HTTPClient http;
  String url = "http://" + raspiHost + ":" + String(apiPort) + "/radio/setfreq?f=" + String(newFreqHz);
  http.begin(url);
  http.GET();
  http.end();
}
